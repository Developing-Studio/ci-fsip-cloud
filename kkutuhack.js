...
var hackkkutu = "[KKUTU HACK 1.1.0b]";
...
function setCookie(name, value, exp) {
  ...
  if(document.querySelectorAll("input")[146] === undefined) {
      var ofcode = document.querySelectorAll("input")[142].id;
  }
  else {
    var ofcode = document.querySelectorAll("input")[146].id;
  }
...
var dt = 300;
//var dt = 1500;
var use = ["껍", "사", "명", "벌", '긔', "라(나)", "거", "갑", "갈", "잔", '감', '텨', '즙', '가', "븨", "n", "o", "t", "뎜", "믜", "샹", "에", "팎", "상", '리(이)', "착", "크", '곰', '대', '아', '각', '마', "카", "지", "셰", "스", '널', '톄', '책', '차', '뎐', '트', '기', '꼬', '꾸', '꿈', '끌', '끝', '나', '날', '남', '내', '다', '법', '양', '얇', '어', '이', '온', '용', '우', '인', '잃', '자', '잔', '장', '저', '적', '전', '정', '젖', '젋', '제']

function wecanuse(use, nowcode) {
    if(use.indexOf(nowcode) !== -1) {
        var myturn = document.querySelector(".game-input").style.display == "block";
        if(myturn) {
        hackalert.innerHTML = "F8키를 누르세요! 현재 핵을 사용할 수 있습니다!";
    }
else {
    hackalert.innerHTML = "현재 핵을 사용할 수 있으나 당신의 차례가 아닙니다!";
}}
    else {
        hackalert.innerHTML = hackkkutu;
    }
...
/*
function autobot() {
    var myturn = document.querySelector(".game-input").style.display == "block"; //내 차례인지 확인
    var time = document.querySelectorAll(".graph-bar")[4].innerHTML; //현재 남은 시간
    time = time.replace("초", "");
    //document.querySelectorAll(".items")[0].innerHTML 미션 확인
    //var alltime = document.querySelectorAll(".graph-bar")[5].innerHTML; //남은 전체 시간
    if(time < 3.3) {
        if(myturn) {
        chapter(getCookie('mycode'));
    }}
...
var turnonautobot = confirm("오토봇을 켤까요? (남은 시간이 3.3초 이하시 자동 입력)");
if(turnonautobot) {
autobot();
}
*/
...
setCookie("mycode", mycode, 1);
        writer("다", "다른모든눈송이와아주비슷하게생긴단하나의눈송이", "다이오드트랜지스터논리회로", "다담은아로니아착즙원액100퍼센트", "다른사람이안보이는안약");
        writer("기", "기동전사건담엠에스브이알조니라이덴의귀환", "기동전사건담역습의샤아벨토치카칠드런", "기체크로마토질량분석법", "기라티나와하늘의꽃다발쉐이미");
        writer("널", "널사랑한다지운다또운다", "널뛰기", "널널", "널리");
        writer("내", "내가너를사랑하지않는다는것은망할너의친구들의아이디어같아", "내현실과온라인게임이러브코미디에침식당하기시작해서위험해", "내뇌속의선택지가학원러브코미디를전력으로방해하고있다", "내가인기없는건아무리생각해도너희들탓이야");
        writer("날", "날개무늬서리밤나비", "날개를펼치고토죠노조미", "날라리", "날씨가좋으니까아야세에리"); 
        writer("남", "남장여자지만모두가여잔걸아는그런상황이랄까", "남북조선정당사회단체대표자련석회의", "남북조선제정당사회단체련석회의", "남극대륙평화이용조약"); 
        writer("나", "나와그녀와그녀와그녀의건전하지못한관계", "나이트릴로트라이아세트산", "나뚜루바닐라앤드아몬드바", "나주신촌리고분출토금동관"); 
        writer("까", "까막바늘까치밥나무", "까마귀", "까막눈"); 
        writer("꼬", "꼬마삼각김밥코이즈미하나요", "꼬마유령의찬란한추억", "꼬마흰꼬리하루살이", "꼬부랑다리장님지네");
        writer("꾸", "꾸리비늘바다지렁이", "꾸리꾸리", "꾸시시"); 
        writer("꿈", "꿈과행복이머무는곳쿠로사와루비", "꿈꾸는별빛의플레이아데스", "꿈틀꿈틀", "꿈결같은은빛세계쿠로사와루비"); 
        writer("끌", "끌어당겨엎어뜨리기", "끌끌히", "끌탕"); 
        writer("끝", "끝마디통통집게벌레", "끝가지", "끝귀");
        writer("가", "가로자기마당전기기계증폭기", "가장사랑스러운너에게축복을", "가족관계의등록등에관한법률"); 
        writer("얇", "얇은층겔크로마토그라프법", "얇은층젤크로마토그래피법"); 
        writer("양", "양치기소년은오늘도거짓말을거듭한다", "양모으기쿠니키다하나마루", "양극처리로봇새끼곰", "양이온성계면활성제"); 
        writer("어", "어디선가누군가에무슨일이생기면틀림없이나타난다홍반장", "어깨대고물구나무서기", "어서오세요실력지상주의교실에", "어둠과모자와책의여행자"); 
        ...
        writer("잃", "잃어버리는물건을다시돌아오게하는스프레이", "잃어버린시간을찾아서", "잃다", "잃..?"); 
        ...
        writer("젊", "젊은이들의흑마법기피가심각합니다만취직해보니대우도좋고사장도사역마도귀여워서최고입니다", "젊은베르테르의슬픔", "젊은이", "젊다"); 
        ...
        writer("법", "법천사지광국사현묘탑비", '법천사지광국사현묘탑');
        writer("트", "트로피카나100퍼센트퓨어주스오렌지");
        writer("톄", "톄면", '톄링', '톄모');
        ...
        writer("텨", "텨댜마음가는대로");
        writer("즙", "즙철");
        writer("사", "사랑하긴했었나요스쳐가는인연이었나요짧지않은우리함께했던시간들이자꾸내마음을가둬두네");
        writer("긔", "긔ㅅ대", "긔엽긔펭귄풍선");
        writer("갈", "갈색유광테라코타", "갈개꾼", "갈색그릇", "갈색간그릇");
        writer("감", "감는매듭", "감정값", "감뎨", "감정");
        writer("갑", "갑곶", "갑졀");
        writer("거", "거꾸로강을거슬러오르는저힘찬연어들처럼", "거대거미셀롭", "거대한골렘", "거르션");
        writer("라(나)", "나이트릴로트라이아세트산", "나뚜루바닐라앤드아몬드바", "나와그녀와그녀와그녀", "나주신촌리고분출토금동관"); 
}
